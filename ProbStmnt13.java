...
public class ProbStmnt13 {
	...
	public static int solution(int[] A, int[] B, int M, int X, int Y) {
		...
		if(A.length != B.length)
			throw  new RuntimeException("Ivalid Array length!");
	...
	static int helper(Queue<Task> taskQueue, int M, int X, int Y) {
	        ...
	        System.out.println(" floors.cardinality(): " +  floors.cardinality());
	        return floors.cardinality() + 1;
	    }

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		int A[] = new int[] { 40, 20, 20, 100,10,200,200 }; // People wt
		int B[] = new int[] { 2, 3, 3, 3, 3,4,3 }; // floor stop
		int M = 3, X = 5, Y = 200;
		System.out.println("output: " + ProbStmnt13.solution(A, B, M, X, Y));
	}
